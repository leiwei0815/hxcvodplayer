#include "hxc_aes256_cbc.h"
#include <string.h>

static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

static const uint8_t Rcon[14] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36,0x6C,0xD8,0xAB
};

static void sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

static void inv_sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) state[i] = rsbox[state[i]];
}

static void shift_rows(uint8_t* state) {
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp; temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;
}

static void inv_shift_rows(uint8_t* state) {
    uint8_t temp;
    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[10]; state[10] = state[2]; state[2] = temp; temp = state[14]; state[14] = state[6]; state[6] = temp;
    temp = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = state[3]; state[3] = temp;
}

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    uint8_t hi_bit_set;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set) a ^= 0x1B;
        b >>= 1;
    }
    return p;
}

static void mix_columns(uint8_t* state) {
    uint8_t tmp[16];
    memcpy(tmp, state, 16);
    for (int i = 0; i < 4; i++) {
        int idx = i * 4;
        state[idx]   = gmul(0x02, tmp[idx]) ^ gmul(0x03, tmp[idx+1]) ^ tmp[idx+2] ^ tmp[idx+3];
        state[idx+1] = tmp[idx] ^ gmul(0x02, tmp[idx+1]) ^ gmul(0x03, tmp[idx+2]) ^ tmp[idx+3];
        state[idx+2] = tmp[idx] ^ tmp[idx+1] ^ gmul(0x02, tmp[idx+2]) ^ gmul(0x03, tmp[idx+3]);
        state[idx+3] = gmul(0x03, tmp[idx]) ^ tmp[idx+1] ^ tmp[idx+2] ^ gmul(0x02, tmp[idx+3]);
    }
}

static void inv_mix_columns(uint8_t* state) {
    uint8_t tmp[16];
    memcpy(tmp, state, 16);
    for (int i = 0; i < 4; i++) {
        int idx = i * 4;
        state[idx]   = gmul(0x0E, tmp[idx]) ^ gmul(0x0B, tmp[idx+1]) ^ gmul(0x0D, tmp[idx+2]) ^ gmul(0x09, tmp[idx+3]);
        state[idx+1] = gmul(0x09, tmp[idx]) ^ gmul(0x0E, tmp[idx+1]) ^ gmul(0x0B, tmp[idx+2]) ^ gmul(0x0D, tmp[idx+3]);
        state[idx+2] = gmul(0x0D, tmp[idx]) ^ gmul(0x09, tmp[idx+1]) ^ gmul(0x0E, tmp[idx+2]) ^ gmul(0x0B, tmp[idx+3]);
        state[idx+3] = gmul(0x0B, tmp[idx]) ^ gmul(0x0D, tmp[idx+1]) ^ gmul(0x09, tmp[idx+2]) ^ gmul(0x0E, tmp[idx+3]);
    }
}

static void add_round_key(uint8_t* state, const uint8_t* round_key) {
    for (int i = 0; i < 16; i++) state[i] ^= round_key[i];
}

static void key_expansion(const uint8_t* key, uint8_t* w) {
    int i = 0;
    while (i < 32) { w[i] = key[i]; i++; }
    i = 32;
    uint8_t temp[4];
    while (i < 240) {
        for (int t = 0; t < 4; t++) temp[t] = w[i - 4 + t];
        if (i % 32 == 0) {
            uint8_t k = temp[0]; temp[0] = temp[1]; temp[1] = temp[2]; temp[2] = temp[3]; temp[3] = k;
            for (int t = 0; t < 4; t++) temp[t] = sbox[temp[t]];
            temp[0] ^= Rcon[i / 32];
        }
        for (int t = 0; t < 4; t++) { w[i] = w[i - 32] ^ temp[t]; i++; }
    }
}

static void aes_encrypt_block(const uint8_t* key, const uint8_t* in, uint8_t* out) {
    uint8_t state[16];
    uint8_t round_key[240];
    memcpy(state, in, 16);
    key_expansion(key, round_key);
    add_round_key(state, round_key);
    for (int round = 1; round <= 13; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_key + round * 16);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_key + 14 * 16);
    memcpy(out, state, 16);
}

static void aes_decrypt_block(const uint8_t* key, const uint8_t* in, uint8_t* out) {
    uint8_t state[16];
    uint8_t round_key[240];
    memcpy(state, in, 16);
    key_expansion(key, round_key);
    add_round_key(state, round_key + 14 * 16);
    for (int round = 13; round >= 1; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, round_key + round * 16);
        inv_mix_columns(state);
    }
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, round_key);
    memcpy(out, state, 16);
}

static size_t pkcs7_pad(uint8_t* data, size_t len) {
    uint8_t pad = AES_BLOCK_SIZE - (len % AES_BLOCK_SIZE);
    for (size_t i = 0; i < pad; i++) data[len + i] = pad;
    return len + pad;
}

static size_t pkcs7_unpad(const uint8_t* data, size_t len) {
    if (len == 0) return 0;
    uint8_t pad = data[len - 1];
    return len - pad;
}

size_t aes256_cbc_encrypt(const uint8_t* key, const uint8_t* iv,
                          const uint8_t* in, size_t in_len,
                          uint8_t* out) {
    uint8_t iv_buf[AES_IV_SIZE];
    memcpy(iv_buf, iv, AES_IV_SIZE);
    size_t padded_len = in_len + (AES_BLOCK_SIZE - in_len % AES_BLOCK_SIZE);
    uint8_t padded[padded_len];
    memcpy(padded, in, in_len);
    padded_len = pkcs7_pad(padded, in_len);
    for (size_t i = 0; i < padded_len; i += AES_BLOCK_SIZE) {
        uint8_t block[AES_BLOCK_SIZE];
        for (int j = 0; j < AES_BLOCK_SIZE; j++) block[j] = padded[i + j] ^ iv_buf[j];
        aes_encrypt_block(key, block, out + i);
        memcpy(iv_buf, out + i, AES_BLOCK_SIZE);
    }
    return padded_len;
}

size_t aes256_cbc_decrypt(const uint8_t* key, const uint8_t* iv,
                          const uint8_t* in, size_t in_len,
                          uint8_t* out) {
    if (in_len % AES_BLOCK_SIZE != 0 || in_len == 0) return 0;
    uint8_t iv_buf[AES_IV_SIZE];
    memcpy(iv_buf, iv, AES_IV_SIZE);
    size_t blocks = in_len / AES_BLOCK_SIZE;
    for (size_t i = 0; i < blocks; i++) {
        uint8_t block[AES_BLOCK_SIZE];
        aes_decrypt_block(key, in + i * AES_BLOCK_SIZE, block);
        for (int j = 0; j < AES_BLOCK_SIZE; j++) out[i * AES_BLOCK_SIZE + j] = block[j] ^ iv_buf[j];
        memcpy(iv_buf, in + i * AES_BLOCK_SIZE, AES_BLOCK_SIZE);
    }
    return pkcs7_unpad(out, in_len);
}
