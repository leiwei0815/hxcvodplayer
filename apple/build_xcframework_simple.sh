...
echo "   [player play];"
echo ""
echo "   // 下载（HXCVD，已静态编入同一 XCFramework）"
echo "   #import <HXCPlayer/HXCVDownload.h>"
echo "   NSString *tid = [[HXCVDownloadManager sharedManager]"
echo "       enqueueDownloadWithURL:url error:&err];"
echo ""
echo "3. 注意事项："
echo "   - FFmpeg 和 SoundTouch 已静态链接到框架中"
echo "   - HXCVD 下载模块已编入框架，并链接系统 CoreData.framework（随系统提供，无需再 Embed）"
echo "   - 无需额外导入其他依赖"
